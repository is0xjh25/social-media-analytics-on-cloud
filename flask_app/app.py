...
def my_view_function():
    ...
    db = couch['twitter']
    # Query the view
    view_result = db.view('_design/happiness_doc/_view/score-gcc-view', reduce=True, group=True)
...
def s1():
    ...
    db = couch['twitter']
    # Query the view
    view_result = db.view('_design/happiness_doc/_view/score-month-view', reduce=True, group=True)
    result = [{'key': row.key, 'value': row.value} for row in view_result]
    #keys = [row['key'] for row in result]
    ...
    # ---------------------------------------------------------------------------
    view_result = db.view('_design/happiness_doc/_view/hour-view', reduce=True, group=True)
    result = [{'key': row.key, 'value': row.value} for row in view_result]
    keys = [row['key'] for row in result]
    ave = [row['value']['avg'] for row in result]
...
# @cross_origin()
# def gccWeekend():
#     couch = couchdb.Server('http://admin:admin@172.26.128.215:5984')
#     db = couch['twitter']
#     view_result = db.view('_design/happiness_doc/_view/score-dow-view', reduce=True, group=True)
#     result = [{'key': row.key, 'value': row.value} for row in view_result]
#     # print(result)
...
def gccCount():
    couch = couchdb.Server(f'http://{couch_password}@{master_node}')
    db = couch['twitter']
    view_result = db.view('_design/happiness_doc/_view/state-view', reduce=True, group=True)
    result = [{'key': row.key, 'value': row.value} for row in view_result]
    return jsonify(result)
...
def combinedData():
    ...
    if GLOBAL_STATE:
        ...
        try:
            ...
            # 1st view result
            view_result_1 = db.view('_design/happiness_doc/_view/score-hour-view', reduce=True, group=True)
            result_1 = [{'key': row.key, 'value': row.value} for row in view_result_1]

            # 2nd view result
            view_result_2 = db.view('_design/happiness_doc/_view/score-month-view', reduce=True, group=True)
            result_2 = [{'key': row.key, 'value': row.value} for row in view_result_2]

            # 3rd view result
            view_result_3 = db.view('_design/happiness_doc/_view/score-dow-view', reduce=True, group=True)
...
def s2():
    ...
    # -----------------------------------------------------
    view_result = db.view('_design/happiness_doc/_view/sal-score-view', reduce=True, group=True)
    result = [{'key': row.key, 'value': row.value} for row in view_result]
    keys = [row['key'] for row in result]
    ave = [row['value']['avg'] for row in result]
